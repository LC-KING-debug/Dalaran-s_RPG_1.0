...
app = Flask(__name__)
# Chave estável para sessões no ambiente local
app.secret_key = "dalaran_secret_key_fixed"

# Registra o Blueprint de autenticação diretamente na raiz para o JS achar o caminho limpo
app.register_blueprint(auth_bp)

# Inicializa o banco de dados e cria as tabelas se não existirem
init_db()

# Dicionário global para instâncias em memória enquanto jogam
jogos_ativos = {}

def carregar_jogo_para_memoria(user_id, personagem_row):
    """Reconstrói o objeto Game com base nos dados exatos salvos no banco."""
    # Instancia o Personagem com nome e classe vindos do banco
    p = Personagem(personagem_row['nome'], personagem_row['classe'])
    
    # Restaura o estado salvo convertendo explicitamente para inteiros
    ...
    # Recalcula os multiplicadores de ATK e DEF baseados no nível atual
    for _ in range(1, p.nivel):
        p.ataque = int(p.ataque * 1.15)
        p.defesa = int(p.defesa * 1.15)
    
    # Trata o inventário armazenado
    ...
    # Cria a instância de Game e anexa o jogador reconstruído
    game_instance = Game(p.nome, p.classe)
    game_instance.player = p
    
    # Guarda no dicionário global usando o id do usuário como chave
    jogos_ativos[user_id] = game_instance

def salvar_jogo_no_banco(user_id):
    """Persiste o progresso do personagem de volta ao banco SQLite."""
...
def listar_personagens():
    ...
    # Converte os objetos sqlite3.Row em dicionários comuns legíveis pelo JSON
